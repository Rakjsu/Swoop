...
/// Cria um pacote no fim da lista.
pub fn insert(
    c: &Connection,
    name: &str,
    ...
    c.execute(
        "INSERT INTO packages (name, dest_dir, position, created_at)
         VALUES (?1, ?2, (SELECT COALESCE(MAX(position), 0) + 1 FROM packages), ?3)",
        params![name, dest, now_ms()],
    )?;
...
pub fn get(c: &Connection, id: PackageId) -> Result<Option<PackageRow>, StoreError> {
    Ok(c.query_row(
        "SELECT id, name, dest_dir FROM packages WHERE id = ?1",
        [id.0],
        |r| {
            Ok(PackageRow {
