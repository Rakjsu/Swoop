...
fn divisao_rapida_corta_no_meio() {
    ...
    let adv = t.advance(a.idx, (5 * MIN_SEGMENT) as usize);
...
fn vitima_lenta_perde_quase_tudo() {
    ...
    t.advance(slow.idx, 10_000);
    t.advance(fast.idx, (6 * MIN_SEGMENT) as usize);
    ...
    fn invariante_sob_operacoes_aleatorias(
        ...
        for op in ops {
            ...
            match op {
                ...
                Op::Advance(i, n) => { t.advance(pick(i), n as usize); }
                Op::Write(i) => {
                    let idx = pick(i);
