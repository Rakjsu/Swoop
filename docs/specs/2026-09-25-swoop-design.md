...
- **26/09, correção v0.7.1 (relato do dono no Windows):** arquivo de 1,7 GB do fastfile.cc falhou com "página final do XFileSharing sem o link"; o fastfile pede 60 s mesmo com captcha; antes, um link do fastfile tinha salvo o HTML como arquivo; a atualização automática pediu para fechar.
  - Causa mais provável do erro: o contador real não casava com o seletor (fixtures reconstruídas), o Swoop leu 0 s e enviou o `download2` antes dos 60 s. Correção: contador lido de mais formatos (`#countdown`, `data-seconds`, `<script>`) e, sem número legível, espera padrão de 60 s (`fallback_countdown_secs`) — o envio nunca sai antes. Página final tolerante (meta refresh, `location.href`, link para `sNN.<site>`).
  - Sem acesso ao site daqui: a página que um plugin não entende fica em `<dados>/diagnostico/` (o erro diz onde) e o app passa a gravar `logs/swoop.log`, para o dono mandar e a v0.7.2 trocar as fixtures pela captura real.
  - HTML como arquivo: a sonda recusa página (HTML que não é anexo) — 1 re-resolução e depois falha clara, sem `.part`.
  - Atualização: o setup era aberto antes de o app fechar (até 5 s gravando o progresso) e o NSIS achava o `swoop.exe` vivo; agora o motor para e a janela some antes do setup abrir.
  - Pedido do dono: depois de instalar, apagar o instalador baixado. O app, ao abrir, remove `%TEMP%\swoop-update\` (tenta até 5 vezes, porque o setup que acabou de relançar o app ainda pode segurar o arquivo).
  - Medido na nuvem: XFS falso com o contador escondido (`countdown_hidden=1`) → 0 envios adiantados com a espera padrão; link final por meta refresh e por script resolvido; página desconhecida guardada em `diagnostico/` com o caminho na mensagem; sonda: página → 1 re-resolução e sucesso quando o 2º link é o arquivo, falha "página em vez do arquivo" sem arquivo gravado quando é sempre página, HTML como anexo baixa normal.
